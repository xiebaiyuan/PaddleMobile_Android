    ...
    fun deleteDirWihtFile(dir: File?) {
        dir?.let {
            ...
            for (file in dir.listFiles()) {
                if (file.isFile){
                    Log.d("delete","delete:$file.absolutePath")
                    file.delete() // 删除所有文件
        ...
        try {
            ...
            if (fileNames.isNotEmpty()) {
                ...
                for (fileName in fileNames) {
                    Log.d("copyfiles", "copyFilesFromAssets fileName: $fileName")
                    copyFilesFromAssets(context, "$oldPath/$fileName", "$newPath/$fileName")
                }
            } else {

                // file
                val iss = context.assets.open(oldPath)
                val fos = FileOutputStream(File(newPath))
                iss.copyTo(fos,1024)
                fos.flush()
                CloseIoUtils.closeIO(fos, iss)
